...
function validateDataByte(value: number, name: string) {
  if (!Number.isInteger(value) || value < 0 || value > 127) throw new Error(`The MIDI ${name} must be between 0 and 127.`)
}

export function controlChange(channel: number, control: number, value: number): MidiMessage {
  validateChannel(channel)
  validateDataByte(control, 'control change')
  validateDataByte(value, 'control value')
  return [0xb0 + channel - 1, control, value]
}

export function programChange(channel: number, program: number): MidiMessage {
  validateChannel(channel)
  validateDataByte(program, 'program')
